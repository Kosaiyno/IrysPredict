...
export default async function handler(req){
  ...
  try {
    ...
    for (const m of sample) {
      ...
      const v = r?.result;
      checks.push({ wallet: m.wallet, score: m.score, lastTs: v ?? null });
    }

    const missing = checks.filter(c => !c.lastTs).length;
    return new Response(JSON.stringify({ ok: true, membersCount: members.length, sampleCount: checks.length, missingInSample: missing, sample: checks }), { headers: { 'content-type': 'application/json' } });
  } catch (err) {
